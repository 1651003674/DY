...
class StepByStepOut
{
    ...
    public function save( $DB ){
        ...
        foreach ($data as $line){
            // 获取当前 遍历单据信息
            $dataArray = DocumentsModel::getCloudInfo('STK_TRANSFEROUT', $line['FBillNo'] );
//return json($dataArray);
            $this->saveMasterSingle( $dataArray );
        }

        return returnData('成功同步了 '.count($data).' (张) 分步式调出单...');
    }
    ...
    private function saveMasterSingle( $dataArray ){
        ...
        $ICStockBillMaster =
            [
                ...
                'FDeptID'=>getValue(ItemClassTable::getWiseItemIDByNmae(2,'销售部'),''),//FDeptID	INTEGER	部门// 部门"0",//FDeptID	INTEGER	部门
                'FEmpID'=>"602",// FEmpID	INTEGER	业务员
                'FSupplyID'=>ItemClassTable::getWiseItemID(1,$dataArray['CustID_Id']),// FSupplyID	INTEGER	供应商内码// 供应商"604",// FSupplyID	INTEGER	供应商内码
                ...
                'FDCStockID'=>ItemClassTable::getWiseItemIDByNmae(5,'渠道仓'),// FDCStockID	INTEGER	收入库房// 收料仓库
                'FBillerID'=>"16394",
                'FHookInterID'=>"0",
                'FPosted'=>"0",
                'FCheckSelect'=>"0",
...
//         出入库单据 分录
        ...
        foreach ($InStockEntry as $k=>$v){
            ...
            $ICStockBillEntry = [
                ...
                'FSCStockID'=>"0",
                'FDCStockID'=>ItemClassTable::getWiseItemIDByNmae(5,'渠道仓'),//调入仓库
                'FCostObjGroupID'=>"0",
                'FCostOBJID'=>"0",
                ...
                'FPurchasePrice'=>$priceInfoArr[$v['MaterialID_Id']]['FTaxPrice'],
                'FPurchaseAmount'=>$priceInfoArr[$v['MaterialID_Id']]['FAllAmount'],
