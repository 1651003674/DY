...
class Cloud_webapi_client
{
    private $cloudUrl = "http://118.122.113.114:8082/k3cloud/";
    private $Login_data = [
//        '5c9317a12b6d00',//帐套ID
//        '5c1e8e9cff9168',//帐套ID 南海
        '5c87101bd87fa8',// 印象集团 测试
//        '5cb8206f83c518',// 印象集团
        'administrator', // 用户名
        '888888',// 密码
        2052//语言标识
    ];
    protected $cookie_jar = null;// 登录后 Cookie 存放位置
    function __construct()
    {
        // nginx 环境
//        $this->cookie_jar = tempnam('/usr/share/nginx/html/Kingdee_impressiom/runtime', 'CloudSession');// 登录后 Cookie 存放位置

        // windows 环境
        $this->cookie_jar = tempnam('../runtime/temp/', 'CloudSession');// 登录后 Cookie 存放位置

        if(!$this->login())
        {
            return 'Login Error...:金蝶云登录出错。';
        }
    }
...
//    }
     ...
     * @return mixed
     */
    public function Save($post_content){
       return $this->invoke_save($this->cloudUrl,$post_content,$this->cookie_jar);
    }
     ...
     * @return mixed
     */
    public function View($post_content){
        return $this->invoke_view($this->cloudUrl,$post_content,$this->cookie_jar);
    }
     ...
     * @return mixed\
     */
    public function getList($post_content){
        return $this->invoke_list($this->cloudUrl,$post_content,$this->cookie_jar);
    }
     ...
     * @return mixed
     */
    public function audit($post_content){
        return $this->invoke_audit($this->cloudUrl,$post_content,$this->cookie_jar);
    }
     ...
     * @return mixed
     */
    public function unaudit($post_content){
        return $this->invoke_audit($this->cloudUrl,$post_content,$this->cookie_jar);
    }
     ...
     * @return mixed
     */
    public function submit($post_content){
        return $this->invoke_audit($this->cloudUrl,$post_content,$this->cookie_jar);
    }
    ...
    protected function login()
    {
        ...
        $post_content = $this->create_postdata($this->Login_data);// 创建请求数据体
     ...
     * @return mixed
     */
    protected function invoke_login($cloudUrl,$post_content,$cookie_jar)
    {
