...
/**
 * Class Expensesclaimsheer 费用报销单
 * @package app\impressiom\controller
 */
class Expensesclaimsheer extends Controller
{
    /**
     * @name 费用报销单 同步界面
     * @return mixed
...
//                "FIsIOSForFin=0",// 是否组织间业务 0 = false
                "FExpenseOrgId   IN (".$inputData['orgId'].")",//指定单据 组织
...
//var_dump($kvArr);die();
    ...
    public function Asyc(){
        ...
        try{
            ...
            return ( new DocumentsModel( WiseDB::getConnect( request()->param('orgId') ) ) )->save( [new \app\impressiom\model\Expensesclaimsheer(),'save'] );
        }catch (Exception $e){
